...
app.get('/api/live/:id/detail', (req, res) => {
  try {
    ...
    const data = {
      ...
      viewers_count: parseInt(totalData.total_user_cnt ?? totalData.audience_count ?? totalData.buyer_count ?? totalData.viewerCount ?? totalData.userCount ?? base.total_user_cnt ?? base.audience_count ?? base.buyer_count ?? base.viewerCount ?? base.uniqueViewCount ?? base.userCount ?? baseResp.data?.total_user_cnt ?? baseResp.data?.audience_count ?? baseResp.data?.buyer_count ?? 0) || null,
      viewers_count_label: (() => { const v = totalData.total_user_cnt ?? totalData.audience_count ?? totalData.buyer_count ?? totalData.viewerCount ?? totalData.userCount ?? base.total_user_cnt ?? base.audience_count ?? base.buyer_count ?? base.viewerCount ?? base.uniqueViewCount ?? base.userCount ?? baseResp.data?.total_user_cnt ?? baseResp.data?.audience_count ?? baseResp.data?.buyer_count; return v != null ? String(v) : null })(),
      _debug_base_keys: Object.keys(base).filter(k => !['title','handle','nickname','creator_id','uid','avatar_url','main_category','create_time','finish_time','duration','record_duration','product_count','revenue','sale','views','unit_price','screenshotUrl','shortUrl'].includes(k)),
